...
func TestEIP155Signer_TeleCallPocRequest(t *testing.T) {
	...
	for _, testCase := range testTable {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			signer := NewEIP155Signer(crypto.AllForksEnabled.At(0), big.NewInt(1).Uint64())
...
func TestEIP155Signer_TeleCallInfo(t *testing.T) {
	...
	for _, testCase := range testTable {
		nonce := uint64(0)
		chainId := uint64(2)
		input := `{"peerId": "16Uiu2HAmPkUzkeHwdGWmTJtFC9R4tq2VoNkN77Miv1oSQJBbyZkz","endpoint": "/info","input": ""}`
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			t.Log("from addr: " + address.String())
			t.Log("publickKey: " + crypto.PubKeyToAddress(&privateKey.PublicKey).String())
			signer := NewEIP155Signer(crypto.AllForksEnabled.At(0), chainId)
...
func TestEIP155Signer_TeleCallIdl(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range testTable {
		nonce := uint64(0)
		chainId := uint64(2)
		input := `{"peerId": "16Uiu2HAmPkUzkeHwdGWmTJtFC9R4tq2VoNkN77Miv1oSQJBbyZkz","endpoint": "/idl","input": ""}`
...
func TestEIP155Signer_TeleCallApi(t *testing.T) {
	...
	for _, testCase := range testTable {
      ...
      "alwayson_scripts": {}
			...
			t.Log("from addr: " + address.String())
			t.Log("publickKey: " + crypto.PubKeyToAddress(&privateKey.PublicKey).String())
			signer := NewEIP155Signer(crypto.AllForksEnabled.At(0), big.NewInt(2).Uint64())
...
func TestEIP155Signer_ChainIDMismatch(t *testing.T) {
	...
	for _, chainIDTop := range chainIDS {
		...
		signer := NewEIP155Signer(crypto.AllForksEnabled.At(0), chainIDTop)
		...
		for _, chainIDBottom := range chainIDS {
			signerBottom := NewEIP155Signer(crypto.AllForksEnabled.At(0), chainIDBottom)
