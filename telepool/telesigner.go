...
func NewSigner(forks crypto.ForksInTime, chainID uint64) TxSigner {
	var signer TxSigner
...
func (f *FrontierSigner) Sender(tx *types.Telegram) (types.Address, error) {
	...
	pub, err := crypto.Ecrecover(f.Hash(tx).Bytes(), sig)
	if err != nil {
		return types.Address{}, err
	}
...
func NewEIP155Signer(forks crypto.ForksInTime, chainID uint64) *EIP155Signer {
...
func (e *EIP155Signer) Sender(tele *types.Telegram) (types.Address, error) {
	...
	pub, err := crypto.Ecrecover(e.Hash(tele).Bytes(), sig)
	if err != nil {
		return types.Address{}, err
	}
...
func (e *EIP155Signer) Provider(tele *types.Telegram) (types.Address, error) {
	...
	pub, err := crypto.Ecrecover(tele.RespHash.Bytes(), sig)
	if err != nil {
		return types.Address{}, err
	}
